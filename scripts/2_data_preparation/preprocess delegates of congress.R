...
combined_df <- readRDS("delegates_raw_data.RDS")
...
write.csv(all_data, "csv\\Letters of Delegates.csv", row.names = FALSE)

## B) FOR MAKING WORD2VEC MODELS (SHICO) ---- 
combined_df3 <- combined_df2 %>% select(ID,Year,Title)
write.csv(combined_df3, "raw\\Letters of Delegates.csv", row.names = FALSE)
...
for (i in 1:25) {
    ...
    current_dataframe <- readRDS(file.path("RDS", filename)) %>% 
        select(text) %>% 
        separate(text,c('Volume', 'Content'), sep = '--', extra = "merge") %>% 
        select(Content) %>% 
        rename(text = Content) %>% 
    ...
    # Create a sub-folder for the current data.frame
    #folder_name <- paste0("V_", i)
    ...
    for (j in 1:nrow(current_dataframe)) {
        ...
        # Create the file name for the txt file (e.g. V_1/dg0011.txt")
...
# note: standard cleaning of the letter texts occurs in the python script main.py
# aimed at preprocessing the data to develop word2vec models to be used in shico
...
#     file_name <- paste0("processed/content/", yr, ".csv")
...
# Filter letter data
shico_words_path <- "shico_vocabulary_per_year.csv"
...
write.csv(pp_data, "links.csv", row.names = FALSE)

## H) FOR PLOTTING NETWORKS ----
saveRDS(combined_df2, "delegates_plotting_data.rds")
